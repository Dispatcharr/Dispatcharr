  ...
  ({ children, getValue, isUnlocked, renderLocked }) => {
    if (!isUnlocked) {
      ...
      return renderLocked ? (
        ...
        <div className="ws-nowrap text-overflow-ellipsis overflow-hidden px-4">
          {getValue() ?? ''}
        </div>
      );
    }
...
export const EditableTextCell = ({ row, column, getValue }) => {
  ...
  if (!isUnlocked || !isFocused) {
    return (
      <div
        className={`whitespace-nowrap overflow-hidden text-overflow-ellipsis px-4 ${isUnlocked ? 'cursor-text' : ''}`}
...
export const EditableNumberCell = ({ row, column, getValue }) => {
  ...
  if (!isUnlocked || !isFocused) {
    return (
      <div
        className={`align-right px-4 ${isUnlocked ? 'cursor-text' : ''}`}
        onClick={() => isUnlocked && setIsFocused(true)}
      >
        {formattedValue}
      </div>
...
const EditableLogoCellInner = ({ row, logoId, onBlur }) => {
  ...
  const renderOption = ({ option }) => {
    ...
    return (
      <div className="flex items-center gap-2 py-8 px-12 min-h-[50px]">
        <img
