...
export default class API {
  ...
  static async getChannelsSummary(params = new URLSearchParams()) {
    try {
      const url = `${getHost()}/api/channels/channels/summary/?${params.toString()}`;
  ...
  static async requeryStreams() {
    ...
    try {
      const [response, ids] = await Promise.all([
        request(`${getHost()}/api/channels/streams/?${params.toString()}`),
  ...
  static async getEPGData() {
    try {
      const response = await request(`${getHost()}/api/epg/epgdata/`);
  ...
  static async getGrid() {
    try {
      const response = await request(`${getHost()}/api/epg/grid/`);
  ...
  static async getEnvironmentSettings() {
    try {
      const response = await request(`${getHost()}/api/core/settings/env/`);
  ...
  static async getBackupStatus(taskId, token = null) {
    try {
      let url = `${getHost()}/api/backups/status/${taskId}/`;
      if (token) {
  ...
  static async getVersion() {
    try {
      const response = await request(`${getHost()}/api/core/version/`, {
        auth: false,
      });
  ...
  static async getVODStats() {
    try {
      const response = await request(`${getHost()}/proxy/vod/stats/`);
  ...
  static async me() {
    return await request(`${getHost()}/api/accounts/users/me/`);
  }
