...
const SystemSettingsForm = React.memo(({ active }) => {
  ...
  useEffect(() => {
    if (settings) {
      ...
      // An error floor already keeps critical, so a level saved as critical
      // means error. Trace is left alone: it is a developer level the control
      // does not offer, and rewriting it here would raise the floor.
    ...
    <Stack gap="md">
      ...
      {!isModular && (
        <>
          <Select
            label="Log Level"
            description="Minimum level kept in the log file and container output, applied instantly. Error keeps critical records too; trace is a developer level, reached by starting the container at DISPATCHARR_LOG_LEVEL=TRACE. The container default follows DISPATCHARR_LOG_LEVEL; levels below it are never generated by the application."
