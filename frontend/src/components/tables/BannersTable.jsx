...
const BannersTable = () => {
  ...
  const executeDeleteBanner = useCallback(
    ...
    [fetchAllBanners, clearSelections]
  );
  ...
  const executeBulkDelete = useCallback(
    ...
    [selectedRows, fetchAllBanners, clearSelections]
  );
  ...
  const executeCleanupUnused = useCallback(
    async (deleteFiles = false) => {
      ...
      try {
        ...
        // Force refresh all banners after cleanup to maintain full view
  ...
  const onBannerSuccess = useCallback(
    async (result) => {
      ...
      if (type === 'update' && banner) {
        ...
        // Note: uploads are handled automatically by API.uploadBanner, so this path is rarely used
        addBanner(banner);
      } else {
