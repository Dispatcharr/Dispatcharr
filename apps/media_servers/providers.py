...
class MediaProviderSession(requests.Session):
    """Keep provider authentication headers on same-origin redirects only."""

    def rebuild_auth(self, prepared_request, response):
        super().rebuild_auth(prepared_request, response)
        source = urlparse(response.request.url)
        target = urlparse(prepared_request.url)
        if (source.scheme, source.netloc) == (target.scheme, target.netloc):
            return
        for header in list(prepared_request.headers):
            if header.lower() in SENSITIVE_PROVIDER_HEADERS:
...
def _prepare_local_artwork(
    ...
    *,
    ...
    if not prefer_nfo:
        return _prefer_tmdb_artwork(
            metadata,
            local_poster=local_poster,
            local_backdrop=local_backdrop,
        )
    if local_poster and not str(metadata.get('poster_url') or '').strip():
        metadata['poster_url'] = local_poster
    if local_backdrop and not str(metadata.get('backdrop_url') or '').strip():
...
class BaseMediaServerClient:
    ...
    def __init__(self, integration: MediaServerIntegration):
        ...
        self.session = MediaProviderSession()
...
class PlexClient(BaseMediaServerClient):
    ...
    def __init__(self, integration: MediaServerIntegration):
        ...
        if self.api_token:
            self.session.headers.update({'X-Plex-Token': self.api_token})
        ...
        for library in libraries:
            ...
            while True:
                ...
                for show in metadata:
                    ...
                    if not episodes:
                        continue
                    provider_series.library_id = library.id
                    for episode in episodes:
                        episode.library_id = library.id
...
class LocalClient(BaseMediaServerClient):
    ...
    def _load_locations(self) -> dict[str, dict]:
        typed_locations = (
            ...
            if getattr(self.integration, "pk", None)
            else []
        )
        provider_config = (
        ...
        raw_locations = (
            ...
            if typed_locations
            else provider_config.get('locations', [])
        )
    ...
    def iter_movies(self, libraries: list[ProviderLibrary]) -> Iterable[ProviderMovie]:
        prefer_nfo = prefer_nfo_metadata()
        for library in libraries:
            if library.content_type not in {'movie', 'mixed'}:
                continue
            ...
            for full_path in self._iter_location_files(
                ...
                metadata, poster_fallback, backdrop_fallback = _prepare_local_artwork(
                    metadata,
        ...
        for library in libraries:
            ...
            for full_path in self._iter_location_files(
                ...
                if not provider_series:
                    ...
                    series_meta, poster_fallback, backdrop_fallback = _prepare_local_artwork(
                        series_meta,
                        prefer_nfo=prefer_nfo,
                        local_poster=local_poster,
                        local_backdrop=local_backdrop,
