...
def stream_xc(request, username, password, channel_id):
    ...
    # Resolve the channel id. xc_get_live_streams returns each channel's
    # provider stream_id (custom_properties.stream_id) instead of the internal
    # Channel.id, so XC clients address the channel by that provider id. Try
    # that lookup first, fall back to the legacy internal-id path. The
    # channel-profile filter below is applied to whatever Channel we resolved,
    # so neither path bypasses access control.
