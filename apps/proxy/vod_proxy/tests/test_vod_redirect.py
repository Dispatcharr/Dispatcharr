...
class StreamVodRedirectTests(SimpleTestCase):
    ...
    @patch(
        "apps.proxy.vod_proxy.views._media_library_target_for_request",
    ...
    @patch("apps.proxy.vod_proxy.views.network_access_allowed", return_value=True)
    def test_media_library_target_stays_proxied_when_default_is_redirect(
        self,
        _network_ok,
        _is_redirect,
        mock_select,
        ...
        response = stream_vod(
            self._request("/proxy/vod/media-library/target/movie/uuid"),
            content_type="movie",
            content_id="uuid",
        ...
        self.assertEqual(response.status_code, 301)
        self.assertIn("/proxy/vod/media-library/target/movie/uuid/", response["Location"])
        mock_idle.assert_not_called()
        mock_select.assert_not_called()
...
class HeadVodRedirectTests(SimpleTestCase):
    ...
    def test_head_vod_adopts_idle_session(
        ...
        with patch("apps.proxy.vod_proxy.views.ProviderRedirectSession.get", return_value=provider_response), \
             patch("apps.proxy.vod_proxy.views.MultiWorkerVODConnectionManager"):
            from apps.proxy.vod_proxy.views import head_vod
