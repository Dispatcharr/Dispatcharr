...
def _compare_versions(a, b):
    ...
    if not a or not b:
        return 0
    pa = [int(x) for x in a.lstrip("v").split(".")]
    pb = [int(x) for x in b.lstrip("v").split(".")]
...
def _unmanage_dropped_slugs(repo, new_manifest_data):
    ...
    whose slug is no longer listed in the repo's manifest."""
    manifest = new_manifest_data.get("manifest", new_manifest_data)
    current_slugs = {p.get("slug") for p in manifest.get("plugins", []) if p.get("slug")}
    dropped = PluginConfig.objects.filter(source_repo=repo).exclude(slug__in=current_slugs)
    count = dropped.update(source_repo=None, slug="")
    if count:
        logger.info(
...
class PluginRepoListCreateAPIView(APIView):
    def get_permissions(self):
        try:
            return [
                perm() for perm in permission_classes_by_method[self.request.method]
    ...
    def post(self, request):
        ...
        # Fetch manifest and extract repo_name
        try:
            ...
            if not registry_name:
                repo.delete()
                return Response(
                    {"error": "Manifest is missing a 'registry_name'. The repo maintainer must set this field."},
                    status=status.HTTP_400_BAD_REQUEST,
                )
            if not repo.is_official and _is_official_sounding(registry_name):
                repo.delete()
                return Response(
                    {"error": f"The registry name '{registry_name}' is not allowed because it may be confused with an official repo."},
                    status=status.HTTP_400_BAD_REQUEST,
                )
...
class PluginRepoPreviewAPIView(APIView):
...
class PluginRepoDetailAPIView(APIView):
    def get_permissions(self):
        try:
            return [
                perm() for perm in permission_classes_by_method[self.request.method]
...
class PluginRepoRefreshAPIView(APIView):
    def get_permissions(self):
        try:
            return [
                perm() for perm in permission_classes_by_method[self.request.method]
...
class AvailablePluginsAPIView(APIView):
    ...
    def get(self, request):
        ...
        for repo in repos:
            if not repo.cached_manifest:
                try:
                    ...
                    registry_name = (manifest_inner.get("registry_name") or "").strip()
                    if not registry_name:
                        logger.warning("Skipping repo %s: missing registry_name", repo.url)
                        continue
                    if not repo.is_official and _is_official_sounding(registry_name):
                        logger.warning("Skipping repo %s: official-sounding name '%s'", repo.url, registry_name)
                        continue
                    repo.cached_manifest = data
                    repo.last_fetched = timezone.now()
                    repo.name = registry_name
        ...
        for repo in repos:
            ...
            for p in repo_plugins:
                ...
                managed = installed_by_slug.get(slug)
                key_match = _sanitize_plugin_key(slug) in installed_by_key
                if managed:
                    is_installed = True
                    ...
                    is_installed = True
                    installed_version = installed_by_key.get(_sanitize_plugin_key(slug))
...
class PluginDetailManifestAPIView(APIView):
...
class PluginInstallFromRepoAPIView(APIView):
    ...
    def post(self, request):
        ...
        if not created:
            ...
            cfg.save(update_fields=["version", "slug", "source_repo", "updated_at"])
...
class PluginRepoSettingsAPIView(APIView):
