...
- **DVR capture playlists keep wall-clock timing.** The capture's HLS muxer now runs with `program_date_time`, so every segment in `index.m3u8` carries `EXT-X-PROGRAM-DATE-TIME`. Both the capture and the final remux use `make_zero`, so the finished file starts at 0 whatever the source said; the playlist is now the one place the broadcast instant of media time 0 survives for a post-processor, and a resumed attempt is already marked there by the `EXT-X-DISCONTINUITY` that `append_list` writes. The recording file itself is unchanged. - Thanks [@v8eta](https://github.com/v8eta)
