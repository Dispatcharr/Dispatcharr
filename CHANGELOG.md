...
- **`recording_finalized` and `recording_failed` events.** `recording_end` fires when the capture stops, before the HLS playlist is remuxed into the final file, so a webhook or plugin acting on it finds no finished recording. `recording_finalized` now fires once the final status is saved and the file is in place, with `status`, `file_path`, `file_url`, `file_size`, `remux_success`, `has_file`, `bytes_written` and `interrupted_reason`; `recording_failed` fires alongside it when there is no playable file (remux failed, or the file is missing or empty). Both are available to Connections subscriptions, plugins and the System Events log. `recording_end` is unchanged. - Thanks [@v8eta](https://github.com/v8eta)
