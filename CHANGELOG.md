...
- **M3U profile credential transforms no longer fall back to base account credentials.** Live, VOD, and catchup resolve per-profile logins through a shared helper (`apps/m3u/credentials.py`). When a profile's search/replace is set but does not match (or times out), playback and profile refresh fail closed instead of silently using the account's base username/password. XC VOD builds `/movie/` and `/series/` URLs from the same transformed credentials as Live, so Live-shaped patterns still substitute correctly for VOD. Profiles in a ServerGroup whose login cannot be fingerprinted after a failed transform are denied shared-pool capacity rather than streaming unmetered. (Fixes #1635)
...
- **DVR recordings no longer drop ~40 ms of audio (and 2 video frames) at every 4 s segment splice.** Finalized MKVs played fine in Chrome but in Firefox the audio cut out and back in every few seconds. The HLS segments themselves are lossless; the discontinuities were introduced at finalize time by the ffmpeg concat *demuxer*, which demuxes each segment as a separate mpegts input and re-times each file against an estimated duration (`prev.start + duration`), baking a ~40-60 ms A/V gap plus ~2 dropped video frames into the MKV at every splice (a 10-min recording carried 150 of them — ~6 s of audio missing in total). The finalize step now bit-concatenates the segments (playlist order) and remuxes them with a single mpegts demuxer pass, which keeps the continuous DTS timeline seamless (FFmpeg-restart splices are re-based by the mpegts PCR-discontinuity logic). The output is verified before acceptance (`_dvr_verify_concat_output`: monotonic DTS, max 100 ms gap on the audio track) so stream shapes where a single-pass re-base can under/overshoot — e.g. the 33-bit DTS wrap on >19 h recordings or inconsistent PCR after a restart — fall back to the previous concat-demuxer command, and then to the existing HLS→MP4→MKV chain. Applied in both `run_recording` and startup recovery.
