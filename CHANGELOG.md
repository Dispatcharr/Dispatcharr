...
- **Media Library importing and STRM/NFO export.** A new admin-only Media Library workspace imports normalized movies, series, and episodes from Plex, Emby, Jellyfin, and explicitly allowed local filesystem roots, then makes the resulting VOD available through Dispatcharr's existing playback infrastructure. Integrations support per-library movie/series selection, connection tests, manual and scheduled synchronization, live progress, scan history, cancellation, authoritative stale-relation cleanup, and provider-specific cards and artwork.
  - **Local metadata and artwork.** Local scans use filenames and NFO files without `ffprobe`, with configurable NFO preference and optional TMDB enrichment for metadata and posters. The reusable directory browser canonicalizes and revalidates paths beneath configured import/export roots, rejects traversal and escaping symlinks, and permits creating folders only in writable export locations.
  - **Granular Jellyfin/Emby STRM libraries.** Export targets can select individual movies and TV series or bulk-select movies by category/provider, refresh selected XC series on a configurable schedule, and build media-server-compatible `Movies` / `TV Shows` trees containing STRM and optional NFO files. Series intentionally have no select-all action to avoid excessive provider episode-detail requests. Export manifests safely update and remove only files owned by Dispatcharr.
  - **Scoped, recursion-safe STRM playback.** Generated STRM files contain a revocable target-specific playback identifier instead of administrator, provider, JWT, or API credentials. GET, HEAD, Range, seeks, and reconnects stay on the current multi-worker VOD proxy with network policy, provider capacity, connection accounting, and per-target limits intact. Only active XC and local relations may back an exported URL; content already represented by a Plex, Emby, or Jellyfin import is omitted, imported-server relations are never playback failover candidates, and stale URLs return `503` when no safe relation remains.
  - **Automatic DVR Media Library.** A permanent, disableable DVR source follows the recording library configured in DVR Settings, imports completed recordings into VOD through the local-media pipeline, remains manually synchronizable, and cannot be edited or deleted. Completed recordings receive same-name NFO sidecars from available EPG metadata, use the Media Library TMDB key for safe movie/show poster matching, and revalidate playback files against the configured DVR library instead of unrelated local-import roots.
...
- **Automatic Comskip now publishes final DVR metadata to VOD.** DVR Media Library synchronization waits for automatic Comskip to finish, then refreshes the managed NFO with the final file duration/size and rescans the recording. Cut workspaces are recording-specific and cleaned up, final replacement is atomic, duplicate automatic/manual runs are serialized, worker-loss deliveries can resume, concurrent JSON updates are merged, and skipped or failed Comskip still imports the intact recording.
