...
- **Default `ffmpeg` stream profile copies every input stream and repeats MPEG-TS headers.** The locked profile is now `-fflags +genpts+discardcorrupt -user_agent {userAgent} -i {streamUrl} -map 0 -c copy -mpegts_flags +pat_pmt_at_frames+resend_headers+initial_discontinuity -f mpegts pipe:1`. `-map 0` keeps extra audio, subtitle, and data streams that the old default mapping dropped. `-c copy` writes codecs ffmpeg will not re-encode as private data instead of failing the open. Probe size and analyze duration stay at ffmpeg defaults.
- **Default `streamlink` stream profile muxes to MPEG-TS when ffmpeg is used.** The locked profile is now `--http-header User-Agent={userAgent} --ffmpeg-fout mpegts --stdout {streamUrl} best`. Combined HLS is still copied through without starting ffmpeg. `--ffmpeg-fout` only applies when Streamlink has separate audio and video to mux, and it replaces the matroska default so that path matches the channel buffer. (Closes #237)
- **Locked stream and output profiles cannot be edited through the API.** A change to name, command, parameters, active state, or the lock itself on a `locked=True` profile returns 400 and leaves the row unchanged. The user-agent on a locked stream profile can still be updated. Custom unlocked profiles are unchanged.
- **In-progress DVR recordings are served as HLS event playlists.** Capture now passes `-hls_playlist_type event` with the existing `-hls_list_size 0` and `append_list+omit_endlist` flags, so `index.m3u8` includes `#EXT-X-PLAYLIST-TYPE:EVENT` and still omits `#EXT-X-ENDLIST` until finalize. Players that treat an untyped live playlist as a short sliding window can seek across the whole recording while it is still being written. The playlist response sets `Cache-Control: no-cache` so a cached first fetch does not freeze the timeline. - Thanks [@aethewulf11-ops](https://github.com/aethewulf11-ops)

### Fixed

- **Seeded output profiles copy subtitle PIDs instead of failing the open.** Locked "Media Server (AC3 Audio)" and "Web Player (AAC Audio)" now include `-c:s copy`. Without a subtitle codec, ffmpeg's automatic encoder selection fails on any channel that carries a subtitle PID, because MPEG-TS has no default subtitle encoder (`dvb_teletext`, `dvb_subtitle`, and similar). The flag is a no-op on subtitle-free streams. (Closes #1502) - Thanks [@v8eta](https://github.com/v8eta)
- **Deleting a locked stream or output profile returns 400.** The UI already disabled delete, and the database already rejected it, but a direct API delete raised an unhandled validation error.
- **`REDIS_URL` is used when it is set.** Clients, Channels, the cache, and Celery were still built from `REDIS_HOST`, `REDIS_PORT`, and `REDIS_DB`, so a URL (including a unix socket such as `unix:///var/run/redis/redis.sock?db=7`) was ignored. A non-empty `REDIS_URL` is now the connection; host, port, database, user, password, and `REDIS_SSL*` still apply when it is unset or blank. The startup log and Settings connection panel follow the URL scheme and TLS query params. Celery only rewrites `unix://` (`db` query key to `virtual_host`), so a `db=` in a password or TCP query string is left alone. (Closes #1427) - Thanks [@Holbormon](https://github.com/Holbormon)
...
- **Channels and Streams tables handle larger pages more smoothly.** Page sizes now go up to 500 rows, while off-screen rows, row actions, logos, and live resizing do less work. Pinned headers, stream action tooltips, scroll position when locking Channels, and Safari column resizing are fixed.
- **Settings forms own one settings group each.** Stream, System, and DVR (plus the EPG match modal) load, diff, and save only their own `*_settings` group instead of flattening every group into one form-key namespace. That stops cross-tab saves from rewriting unrelated groups and allows same-named keys under different groups later. Proxy, Network Access, and User Limits already followed this pattern. Mid-save store updates no longer flash old values: Stream, System, DVR, Proxy, Network Access, and User Limits share `useSettingsSaveGuard` (`isSavingRef` + `runSave`). Group parse/diff/save coercion now comes from one per-group field config (typed fields) instead of parallel field lists. DVR comskip upload updates the local form only (the API already persists the path) so a store write cannot wipe unsaved edits on other DVR fields. Proxy and User Limits re-sync the form from the save response like the other tabs.
...
- **Log file size and retention defaults match what operators expect.** Maximum log file size defaults to 5 MB and caps at 20 MB (shared constants from the collector). **Log Files Kept** counts the live file plus rotations (keep of 5 means five files on disk, not five archives plus live), with a minimum of 2. The in-app viewer serves up to 24 MB so every file the collector can produce fits without a silent half-file cut. (#1601) - Thanks [@nagelm](https://github.com/nagelm)
...
- **VOD list sync keeps every category/stream version of the same movie or series within one batch.** When a provider offered the same title under multiple enabled XC VOD categories (different `stream_id` / `series_id`) and those rows landed in the same 1000-item chunk, only the first occurrence got an `M3UMovieRelation` / `M3USeriesRelation`; the rest were dropped even though the canonical Movie/Series matched correctly. Sync now records every distinct provider ID under that identity and creates a relation for each, while still coalescing literal duplicate IDs to one relation. Blank Movie/Series fields (for example description) are also filled from later occurrences in the same batch when the first row was sparse. (Closes #1511) - Thanks [@northernpowerhouse](https://github.com/northernpowerhouse)
- **VOD movie/series import no longer discards a whole batch when one provider row has a blank name.** `Movie.name` / `Series.name` are NOT NULL, and each batch runs in one `transaction.atomic()` block, so an explicit null (or empty/whitespace-only) name used to raise `IntegrityError` and roll back up to 1000 valid neighbors while the refresh still looked successful. Blank names are now skipped with a warning so the rest of the batch imports. (Closes #1586) - Thanks [@thejdubb02](https://github.com/thejdubb02)
- **Bulk "Set TVG-IDs from EPG" and "Set names from EPG" no longer overwrite dummy-assigned channels with the shared placeholder.** A dummy EPG source uses one auto-created `EPGData` row for every channel on that source (`tvg_id` / name describe the source, not a channel). Running either bulk action over those channels collapsed every selected channel onto the same value, so XMLTV/M3U with `tvg_id_source=tvg_id` emitted one shared channel id and clients showed identical listings. Both tasks now exclude `source_type='dummy'` from the queryset. - Thanks [@hdimer](https://github.com/hdimer)
- **Raw image proxy endpoints no longer return 406 for `Accept: image/*`.** Channel and VOD logo `cache`, VOD movie/series/episode `image`, EPG program `poster`, and plugin logos stream raw image responses, but DRF still negotiated `Accept` against JSON-only renderers before the action ran. Native image loaders (for example Apple TV / Alamofire sending `Accept: image/*`) got 406 while browsers and curl (`*/*`) looked healthy. A shared negotiator mixin skips that check for those actions only (plugin logo uses the same negotiator on its single-purpose APIView). (Closes #1541) - Thanks [@thejdubb02](https://github.com/thejdubb02)
- **DVR HLS finalize remuxes the playlist instead of concatenating segments.** Finalize (and expired startup recovery) now feed `index.m3u8` to FFmpeg as a single input (`-c copy`) rather than the concat demuxer, which re-timed each `.ts` file and produced ~40 ms A/V gaps at every segment boundary (audible as periodic blips in Firefox and similar players). Capture still uses `omit_endlist` for live viewing; remux appends `#EXT-X-ENDLIST` when missing so the HLS demuxer treats the list as VOD and finishes. Capture and remux both pass `-map 0:v? -map 0:a?` so every video and audio stream is kept (not only ffmpeg's default best pair) without mapping subtitle/data PIDs that the HLS muxer cannot copy. Shared `_dvr_remux_hls_to_mkv` covers both call sites, keeps the MP4-intermediate fallback (`.dvr_{id}_intermediate.mp4`), and caps the whole remux under a 30-minute budget for slow hosts and future optional encode paths. Expired recovery now also points `file_url` at `/file/` and clears `_hls_dir` after a successful remux. - Thanks [@torhve](https://github.com/torhve) for the diagnosis
- **DVR Redirect captures now use the M3U account User-Agent.** When a channel's stream profile is Redirect, FFmpeg follows the proxy 302 straight to the provider. It previously kept sending `Dispatcharr-DVR/recording-<id>`, which providers often reject. Redirect recordings now use the selected stream's M3U account User-Agent (or the system default when unset), matching live provider connections. Proxied captures still identify as Dispatcharr-DVR so the proxy can tag the client while it continues to use the M3U UA upstream.
- **VOD OpenAPI schema and runtime JSON now match for generated clients.** SerializerMethodField types, provider-info / series-episodes / unified-list / logo action shapes, and `/api/vod/all/` pagination (`next`/`previous` as URL or `null`, matching standard DRF) are declared and coerced through response serializers so OpenAPI clients no longer fail to unmarshal. (Fixes #1660) - Thanks [@nshttpd](https://github.com/nshttpd)
- **VOD category `m3u_account` filter uses the correct relation path.** Categories are linked to accounts via `m3u_relations`, not a direct `m3u_account` FK, so `?m3u_account=` on `/api/vod/categories/` filters through `m3u_relations__m3u_account__id`. (Fixes #1661) - Thanks [@nshttpd](https://github.com/nshttpd)
