...
- **`recording_end` now fires after post-processing, with outcome details.** It previously fired when the capture's ffmpeg stopped, before the HLS playlist was remuxed into the final file, so a webhook or plugin acting on it found no finished recording. It now fires once the final status is saved and the file is in place (may arrive later than before), with `outcome` (`success`/`failed`), `has_file`, `failure_reason` (`remux_failed`/`missing_file`/`empty_file`/`null`), `status`, `interrupted_reason`, `file_path`, `file_name`, `file_url`, `file_size`, `remux_success`, `bytes_written`, `start_time` and `end_time`. Also now emitted for recordings finalized by crash recovery on startup. - Thanks [@v8eta](https://github.com/v8eta)
