...
- **DVR path templates support broadcast-date placeholders.** `{start_date}` (`YYYY-MM-DD`), plus `{start_year}`, `{start_month}` and `{start_day}`, are available to the TV, TV fallback, movie, and movie fallback templates, so recordings without a season/episode number can still be named in a form media servers parse — for example `TV/{show}/Season {start_year}/{show} - {start_date} - {sub_title}.mkv`. The values are the programme's broadcast date rendered in the configured system time zone, the same zone the recurring-rule scheduler uses to decide which day a recording belongs to, so a date in the filename cannot disagree with the day the rule matched. The numeric parts are integers, like `{season}`, so `{start_month:02d}` pads the way `{season:02d}` does. They are deliberately distinct from `{original_air_date}`, which stays exclusive to the TV fallback template, and `{start}`/`{end}` keep their existing UTC form. (Closes #682) - Thanks [@v8eta](https://github.com/v8eta)
